...
public class MavenResolverRunnable implements Runnable {
  ...
  MavenPathResolver getMavenPathResolver(ConfigurationService configService, String scmLocation) {
    ...
    List<ScmLocationMappingConfig> mappings = mapper.getMappings(configService, "git", ScmLocationMapper.PURPOSE_BROWSE); //$NON-NLS-1$
    if (mappings.isEmpty()) {
      if (LOG.isLoggable(Level.FINE)) {
        LOG.fine(MessageFormat.format("no suitable scm mapping found matching filter provider=''git'' && purpose=''{0}''",
    ...
    if (LOG.isLoggable(Level.FINE)) {
      LOG.fine(MessageFormat.format("no suitable scm mapping found matching scmLocation=''{0}'' && purpose=''{1}''",
