...
class ProjectEditPanelEntry extends CustomComponent {
  ...
  public ProjectEditPanelEntry(Project project, ExtensionFormService<?> formService, ProjectEditContext context, Application application) {
  ...
  public void markAsInvalid(boolean isInvalid) {
    if (isInvalid) {
      ...
      if (StringUtils.isNotBlank(icon)) {
        trayIcon.setSource(new StreamResource(new ExtensionStreamSource(formService.getClass(), icon), FilenameUtils.getName(icon), application));
      }
    ...
    if (isInvalid && issues != null && issues.size() > 0) {
      for (Issue issue: issues) {
        ...
        if (propertyId != null) {
          Field field = form.getField(propertyId);
          if (field != null) {
            setComponentError(field, new UserError(issue.getMessage(), UserError.CONTENT_UIDL, ErrorMessage.ERROR));
          }
        }
      }
