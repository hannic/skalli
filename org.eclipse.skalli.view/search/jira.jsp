...
<title>ProjectPortal - Add Project to JIRA</title>
<style type="text/css">
...
Click on the link below to open a JIRA dialog on the JIRA server in another browser window. Don't close 
the current window and process the form in the JIRA window. You may want to use the following data:<br/>
<br/>
...
<br/>
Copy the key of the created JIRA project 
to the clipboard before you close the JIRA browser window.
<br/><br/>
<a href="${remoteJiraUrl}" target="_blank">
Follow this link to open a JIRA dialog on JIRA server</a>
<br/><br/>
Paste the copied project key from clipboard to the following input field. Choose if 
you want to set the JIRA
project as BugTracker and/or as Scrum Backlog to this project '${project.name}' and
click on 'Save' if you want to persist this data. 
Click on 'Cancel' if you don't want to save any data to your project. <br/><br/>
...
<c:choose>
  ...
  <c:otherwise>
    <c:if test="${bugTracker != null && addBugTracker}">
      ...
      Bug Tracker is already defined for this project ('${bugTracker }'), click on 'Save' to overwrite 
      the current setting.
      </div>
    </c:if>
    <c:if test="${scrumBacklog != null && addScrumBacklog}">
      <div class="warningMessage">
      Scrum Backlog is already defined for this project ('${scrumBacklog }'), click on 'Save' to overwrite 
      the current setting.
      </div>
    </c:if>
