  ...
  protected PersistenceService getPersistenceService() {
    if (persistenceService == null) {
      throw new IllegalStateException(MessageFormat.format(
          "ERROR: No implementation for required service {0} registered",
    ...
    for (Issue issue: issues) {
      ...
      if (!issue.getEntityId().equals(entityId)) {
        ...
        LOG.warning(MessageFormat.format(
            "Invalid issue detected (requested entity={0} but found entity={1})",
      ...
      if (minSeverity.compareTo(issue.getSeverity()) < 0) {
        invalidIssues.add(issue);
        LOG.warning(MessageFormat.format(
            "Invalid issue detected (requested minSeverity={0} but found severity={1})",
            minSeverity, issue.getSeverity()));
      }
