    ...
    public void write(OutputStream paramOutputStream) throws IOException {
      ...
      if (workDirProperty != null) {
        ...
        if (!parent.exists() || !parent.isDirectory()) {
          LOG.warning(String.format(
              "Working directory (%s) not found. Using current directory as fallback.", parent.getAbsolutePath())); //$NON-NLS-1$
          parent = null;
        }
      }
      ...
      try {
        while (files.hasNext()) {
          ...
          ZipEntry entry = new ZipEntry(StringUtils.substringAfter(FilenameUtils.separatorsToUnix(file.getPath()),
