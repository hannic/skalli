...
public class LuceneIndex<T extends EntityBase> {
  ...
  private String doHighlight(final Highlighter highlighter, final List<String> fields, final String fieldName, String fieldContents) throws IOException {
    String ret = fieldContents;
    if (fieldContents != null && fields.contains(fieldName)) {
      try {
        String[] fragments = highlighter.getBestFragments(analyzer, fieldName, fieldContents, NUMBER_BEST_FRAGMENTS);
  ...
  private SearchHit<T> getSearchHit(final Document doc, final List<String> fields, final Highlighter highlighter) throws IOException {
    ...
    for (Fieldable f : doc.getFields()) {
      ...
      if (fields.contains(f.name())) {
        for (int i = 0; i < highlightedFieldContents.size(); i++) {
          highlightedFieldContents.set(i, doHighlight(highlighter, fields, f.name(), highlightedFieldContents.get(i)));
        }
      }
  ...
  public SearchResult<T> search(final String[] fields, final String queryString, PagingInfo pagingInfo) throws QueryParseException {
  ...
  public FacetedSearchResult<T> facetedSearch(final String[] fields, String[] facetFields, final String queryString, PagingInfo pagingInfo) throws QueryParseException {
    FacetedSearchResult<T> ret = new FacetedSearchResult<T>();
    search(fields, facetFields, queryString, pagingInfo, ret);
    return ret;
  }

  private <R extends SearchResult<T>> R search(final String[] fields, String facetFields[], final String queryString, PagingInfo pagingInfo, R ret) throws QueryParseException {
    ...
    if (StringUtils.equals("*", queryString) || StringUtils.isEmpty(queryString)) { //$NON-NLS-1$
      ...
      List<T> sublist = allEntities.subList(Math.min(pagingInfo.getStart(), allEntities.size()), Math.min(pagingInfo.getStart() + pagingInfo.getCount(), allEntities.size()));
      ...
      try {
        ...
        if (facetFields == null) {
          ...
          collector = new FacetedCollector(facetFields, searcher.getIndexReader(), pagingInfo.getStart() + pagingInfo.getCount());
        }
