...
public class ProjectServiceImpl extends EntityServiceImpl<Project> implements ProjectService {
  ...
  * {@link org.eclipse.skalli.model.core.ProjectTemplate project template}.
  ...
  protected SortedSet<Issue> validateEntity(Project project, Severity minSeverity) {
    ...
    issues.addAll(new ProjectDescriptionValidator(Project.class, Project.PROPERTY_DESCRIPTION).validate(projectUUID, project.getDescription(), minSeverity));
  ...
  private ProjectTemplate validateProjectTemplate(Project project, Set<Issue> issues) {
    ...
    if (projectTemplate == null) {
      issues.add(new Issue(Severity.FATAL, ProjectService.class, project.getUuid(),
          MessageFormat.format("Project references project template ''{0}'' but there is no such template registered",
  ...
  private void validateCompatibility(Project project, ProjectTemplate projectTemplate, ExtensionService<?> extensionService, Set<Issue> issues) {
    ...
    if (allowed != null || included != null || excluded != null) {
      ...
      for (ExtensionEntityBase extension: project.getAllExtensions()) {
        ...
        if (allowed != null && !allowed.contains(projectTemplate.getId())) {
          issues.add(new Issue(Severity.ERROR, ProjectTemplate.class, projectUUID, extension.getClass(), null,
              MessageFormat.format("{0} projects are not compatible with ''{1}'' extensions. Disable the extension or " +
                  "select another project template.", projectTemplate.getDisplayName(), extensionClassName)));
        }
        if (excluded != null && excluded.contains(extensionClassName) ||
            included != null && !included.contains(extensionClassName)) {
          issues.add(new Issue(Severity.ERROR, ProjectTemplate.class, projectUUID, extension.getClass(), null,
              MessageFormat.format("''{0}'' extensions are not appropriate for {1} projects. Disable the extension or " +
                  "select another project template.", extensionClassName, projectTemplate.getDisplayName())));
        }
      }
    }
  }

  private void validateExtension(UUID projectUUID, ExtensionEntityBase ext, ProjectTemplate projectTemplate, Set<Issue> issues, Severity minSeverity) {
  ...
  private void validateExtension(UUID projectUUID, ExtensionEntityBase ext, ExtensionService<?> extensionService, ProjectTemplate projectTemplate, Set<Issue> issues, Severity minSeverity) {
    ...
    for (String propertyName: propertyNames) {
      ...
      if (defaultValidators == null) {
        LOG.warning(MessageFormat.format("{0}#getPropertyValidators({1}) returned null, but is expected to return an empty set",
            extensionService.getClass().getName(), propertyName));
      } else {
        propertyValidators.addAll(defaultValidators);
      }
      Set<PropertyValidator> customValidators = projectTemplate.getPropertyValidators(extensionClassName, propertyName);
      if (customValidators == null) {
        LOG.warning(MessageFormat.format("{0}#getPropertyValidators({1}, {2}) returned null, but is expected to return an empty set",
    ...
    if (extensionValidators == null) {
      LOG.warning(MessageFormat.format("{0}#getExtensionValidators() returned null, but is expected to return an empty set",
  ...
  protected void updateSearchIndex(Project project) {
    ...
    if (searchService != null) {
      ...
      LOG.warning(MessageFormat.format("Failed to update search index - no instance of {0} available", SearchService.class.getName()));
    }
  }
  ...
  public Project createProject(final String templateId, final String userId) {
    ...
    if (template != null) {
      for (Class<? extends ExtensionEntityBase> extensionClass : projectTemplateService.getSelectableExtensions(template, null)) {
        ...
        if (template.isEnabled(extensionClassName)) {
          try {
            ...
            LOG.warning(MessageFormat.format("Extension ''{0}'' could not be instantiated: {1}", extensionClassName, e.getMessage()));
