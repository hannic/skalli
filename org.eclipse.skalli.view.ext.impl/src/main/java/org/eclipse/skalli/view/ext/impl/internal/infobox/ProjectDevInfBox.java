...
public class ProjectDevInfBox extends AbstractInfoBox implements ProjectInfoBox {
  ...
  @SuppressWarnings("nls")
  private void iconizedLinks(final StringBuffer sb, final String icon, final String firstLabel, final String otherLabel, final Set<String> links) {
    if (links.size() == 1) {
      ...
      for (String url : links) {
        ...
        sb.append("<a href=\"").append(url).append("\" target=\"_blank\"").append(isFirst ? ">"  : " class=\"leftMargin\">").append(label).append("</a>");
