...
class AlwaysTrustSSLProtocolSocketFactory implements ProtocolSocketFactory {
  ...
  @Override
  public Socket createSocket(final String hostname, final int port, final InetAddress localAddress, final int localPort) throws IOException, UnknownHostException {
    return getSSLContext().getSocketFactory().createSocket(hostname, port, localAddress, localPort);
  }

  @Override
  public Socket createSocket(final String hostname, final int port, final InetAddress localAddress, final int localPort, final HttpConnectionParams params) throws IOException, UnknownHostException, ConnectTimeoutException {
