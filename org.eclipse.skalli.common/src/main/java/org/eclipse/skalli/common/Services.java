...
public class Services {
    ...
    private static <T> T getService(Class<T> serviceClass, boolean required, String filter) {
        ...
        try {
            ...
            if (serviceRefs != null) {
                if (serviceRefs.length > 1) {
                    throw new IllegalStateException("ERROR: Multiple implementations for service "
                            + serviceClass.getName() + " registered");
                }
        ...
        if (required && serviceInstance == null) {
            throw new IllegalStateException("ERROR: No implementation for required service " + serviceClass.getName()
                    + " registered");
        }
