...
public class StringLengthValidator extends AbstractPropertyValidator {
   ...
   */
  public StringLengthValidator(Severity severity, Class<? extends ExtensionEntityBase> extension, String property, int minLength, int maxLength) {
  ...
  protected String getInvalidMessageFromCaption(Object value) {
    ...
    if (minLength == maxLength) {
      ...
      message = MessageFormat.format("{0} must be at least {1} and at max {2} characters long", caption, minLength, maxLength);
    }
    return message;
  }
  ...
  protected String getDefaultInvalidMessage(Object value) {
    String message = null;
    if (minLength == maxLength) {
      message = MessageFormat.format("Value of property ''{0}'' must be exactly {1} characters long", propertyName, minLength);
    } else if (minLength == 0 && maxLength > 0) {
      message = MessageFormat.format("Value of property ''{0}'' must be at max {1} characters long", propertyName, maxLength);
    } else if (minLength > 0 && maxLength == Integer.MAX_VALUE) {
      message = MessageFormat.format("Value of property ''{0}'' must be at least {1} characters long", propertyName, minLength);
    } else if (minLength > 0 && maxLength > 0) {
      message = MessageFormat.format("Value of property ''{0}'' must be at least {1} and at max {2} characters long", propertyName, minLength, maxLength);
    }
    return message;
  }
