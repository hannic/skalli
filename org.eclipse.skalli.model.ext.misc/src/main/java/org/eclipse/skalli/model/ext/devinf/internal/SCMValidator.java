...
public class SCMValidator implements PropertyValidator, Issuer {
  ...
  private static final Pattern PATTERN_PERFORCE = Pattern.compile("^scm:perforce:perforce(\\d+)(\\.wdf\\.sap\\.corp)?:\\1://.*/$"); //$NON-NLS-1$
        ...
        if (scmLocation.startsWith("scm:perforce:")) { //$NON-NLS-1$
          if (!PATTERN_PERFORCE.matcher(scmLocation).matches()) {
            issues.add(new Issue(Severity.ERROR, SCMValidator.class, entity,
                ...
                MessageFormat.format("''{0}'' is not a valid value for {1}. For Perforce repositories the value " +
          ...
          if (!PATTERN_GIT.matcher(scmLocation).matches()) {
            issues.add(new Issue(Severity.ERROR, SCMValidator.class, entity,
                DevInfProjectExt.class, DevInfProjectExt.PROPERTY_SCM_LOCATIONS, item,
                MessageFormat.format("''{0}'' is not a valid value for {1}. For Git repositories the value " +
          ...
          if (!PATTERN_SVN.matcher(scmLocation).matches()) {
            issues.add(new Issue(Severity.ERROR, SCMValidator.class, entity,
                DevInfProjectExt.class, DevInfProjectExt.PROPERTY_SCM_LOCATIONS, item,
                MessageFormat.format("''{0}'' is not a valid value for {1}. For Subversion repositories the value " +
                    "must match the regular expression {2}.",
          ...
          issues.add(new Issue(Severity.FATAL, SCMValidator.class, entity,
              DevInfProjectExt.class, DevInfProjectExt.PROPERTY_SCM_LOCATIONS, item,
              MessageFormat.format("''{0}'' is not a valid value for {1}. It must start with 'scm:&lt;repotype&gt;:' "
                  + "where &lt;repotype&gt; is one of git, perforce, or svn", scmLocation, caption)
              ));
        }
      }
    }
